...
if __name__ == '__main__':
    ...
    discriminator.load_state_dict(torch.load("./epochs/discriminator-100.pkl"))
    generator = Generator()
    generator.load_state_dict(torch.load("./epochs/generator-100.pkl"))
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for train_batch, train_clean, train_noisy in train_bar:
            ...
            tf.io.write_file('loss.txt',contents=clean_loss)
            tf.io.write_file('loss.txt',contents=noisy_loss)
            tf.io.write_file('loss.txt',contents=g_loss)
            f=open('loss.txt','w')
            f.write('\n')
            f.close
        ...
        g_path = os.path.join('epochs', 'generator-{}.pth'.format(epoch + 1))
        d_path = os.path.join('epochs', 'discriminator-{}.pth'.format(epoch + 1))
